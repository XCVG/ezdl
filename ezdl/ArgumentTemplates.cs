...
namespace ezdl
{
    ...
    {
        public static readonly string Default = "--compat-options youtube-dl --force-ipv4 -ciw -o \"{OutputPath}/video.%(ext)s\" --check-formats --socket-timeout 2 --parse-metadata \"title:%(meta_title)s\" --parse-metadata \"uploader:%(meta_artist)s\" --parse-metadata \"%(channel_id)s:%(meta_channel_id)s\" --write-info-json --add-metadata --merge-output-format mkv {Url}";

        public static readonly string YouTube = "--compat-options youtube-dl {Cookies} --force-ipv4 -ciw -o \"{OutputPath}/video.%(ext)s\" -o \"thumbnail:{OutputPath}/thumbnail.%(ext)s\" {Format} --check-formats --socket-timeout 2 --parse-metadata \"title:%(meta_title)s\" --parse-metadata \"uploader:%(meta_artist)s\" --parse-metadata \"%(channel_id)s:%(meta_channel_id)s\" --write-info-json --add-metadata --write-sub --embed-subs --all-subs --convert-subs=srt --write-thumbnail --merge-output-format mkv {Comments} {Url}";
